...
const WORKSPACE_MCP_JSON = `${JSON.stringify(
  { mcpServers: { grove: { command: "grove", args: ["mcp", "connect"] } } },
  null,
  2,
)}\n`;
...
export async function generateAgentFiles(
  ...
  const mcpJsonStatus = await ensureWorkspaceEntryFile(
    paths.mcpJson(workspace),
    WORKSPACE_MCP_JSON,
  );
