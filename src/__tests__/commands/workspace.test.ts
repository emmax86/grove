...
describe("workspace commands", () => {
  ...
  it("add creates CLAUDE.md bootstrap shim in workspace root", async () => {
    await addWorkspace("myws", paths);
    expect(await exists(paths.claudeMd("myws"))).toBe(true);
    expect(await readFile(paths.claudeMd("myws"), "utf-8")).toBe(WORKSPACE_AGENT_BOOTSTRAP_SHIM);
  });

  it("add creates AGENTS.md bootstrap shim", async () => {
    await addWorkspace("myws", paths);
    expect(await exists(paths.agentsMd("myws"))).toBe(true);
    expect(await readFile(paths.agentsMd("myws"), "utf-8")).toBe(WORKSPACE_AGENT_BOOTSTRAP_SHIM);
  });

  it("add creates .mcp.json pointing at the grove bridge", async () => {
    await addWorkspace("myws", paths);
    expect(await exists(paths.mcpJson("myws"))).toBe(true);
    const config = JSON.parse(await readFile(paths.mcpJson("myws"), "utf-8"));
    expect(config.mcpServers.grove).toEqual({ command: "grove", args: ["mcp", "connect"] });
  });

  it("sync backfills .mcp.json for a workspace created before it existed", async () => {
    await addWorkspace("myws", paths);
    // Simulate a workspace created before .mcp.json generation existed
    await rm(paths.mcpJson("myws"), { force: true });
    expect(await exists(paths.mcpJson("myws"))).toBe(false);
