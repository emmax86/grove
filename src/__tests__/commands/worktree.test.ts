...
describe("worktree commands", () => {
	...
	it("add to second workspace reuses pool entry", async () => {
		...
		const ws1Link = await readlink(
			paths.worktreeDir("myws", "myrepo", "feature-shared"),
		);
		const ws2Link = await readlink(
			paths.worktreeDir("otherws", "myrepo", "feature-shared"),
		);
	...
	it("worktrees.json updated on add and remove", async () => {
		...
		const afterAdd = JSON.parse(
			await readFile(paths.worktreePoolConfig, "utf-8"),
		);
	...
	it("cross-workspace lifecycle: shared pool entry", async () => {
		...
		// ws1 symlink is gone
		const ws1Gone = !(await exists(
			paths.worktreeDir("myws", "myrepo", "feature-cross"),
		));
		...
		expect(
			(
				await lstat(paths.worktreeDir("otherws", "myrepo", "feature-cross"))
			).isSymbolicLink(),
		).toBe(true);
	...
	it("addWorktree rollback: removes symlink and pool entry when addPoolReference fails", async () => {
		...
		expect(
			await entryGone(paths.worktreeDir("myws", "myrepo", "feature-pool-fail")),
		).toBe(true);

		// Pool entry should have been rolled back
		expect(
			await exists(paths.worktreePoolEntry("myrepo", "feature-pool-fail")),
		).toBe(false);
	});
	...
	describe("pruneWorktrees", () => {
		...
		it("does not prune valid pool symlinks", async () => {
			...
			expect(
				(
					await lstat(paths.worktreeDir("myws", "myrepo", "feature-y"))
				).isSymbolicLink(),
			).toBe(true);
		});
		...
		it("does not prune linked (default-branch) symlinks", async () => {
			...
			expect(
				(
					await lstat(paths.worktreeDir("myws", "myrepo", "main"))
				).isSymbolicLink(),
			).toBe(true);
		});
		...
		it("does remove only this workspace's symlink when pool entry is dangling", async () => {
			...
			expect(
				(
					await lstat(paths.worktreeDir("otherws", "myrepo", "feature-shared"))
				).isSymbolicLink(),
			).toBe(true);
		});
		...
		it("does continue pruning remaining entries when rm fails for one entry", async () => {
			...
			expect(
				(
					await lstat(paths.worktreeDir("myws", "myrepo", "feature-perm"))
				).isSymbolicLink(),
			).toBe(true);

			// myrepo2's symlink is gone
			expect(
				await entryGone(paths.worktreeDir("myws", "myrepo2", "feature-perm")),
			).toBe(true);
		});
	});
	...
	describe("setup after worktree add", () => {
		...
		it("setup failure is non-fatal — worktree is still created", async () => {
			...
			expect(
				await exists(paths.worktreePoolEntry("myrepo", "setup-fails")),
			).toBe(true);
			expect(
				(
					await lstat(paths.worktreeDir("myws", "myrepo", "setup-fails"))
				).isSymbolicLink(),
			).toBe(true);
		});
	});
